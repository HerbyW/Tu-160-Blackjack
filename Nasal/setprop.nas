...
# /surface-positions/wing-sweep-pos-norm

# in air: mach contols the setting
# on ground: manual settings

setprop("/position/gear-agl-m", 0);

var sweeptimer = maketimer(0.7, func  {
    if(getprop("/position/gear-agl-m") > 2.5)
    { 
      if (getprop("/velocities/mach") < 0.73 )
      {setprop("/controls/flight/wing-sweep", 0);}
      else
    { 
      if (getprop("/velocities/mach") < 0.93 )
      {setprop("/controls/flight/wing-sweep", 0.33334);}
      else
    {
      if (getprop("/velocities/mach") < 2.1 )
      {setprop("/controls/flight/wing-sweep", 1);}      
    }}}
});
sweeptimer.start();
...
setlistener("/controls/flight/aileron-trim", func(v)
{
  if(v.getValue())
  
    setprop("/controls/flight/taileron", getprop("/controls/flight/aileron-trim") * 0.01);
...
{ 
  if(v.getValue() == 1)
  {  
  setprop("sim/messages/copilot", "Parachutes are deploying!");
  doors.parachutes.toggle()
  }
  else {
  if(v.getValue() == 0)
  {
  setprop("sim/messages/copilot", "Parachutes are gone!");
  doors.parachutes.toggle()
  }}
});
...
setprop("sim/messages/copilot", "Have fun with the Tu-160 White Swan");
...
setlistener("controls/flight/flaps", func
 { 
 if ((getprop("controls/flight/flaps") > 0  ) and (getprop("/instrumentation/airspeed-indicator/indicated-speed-kt") > 260  ))
  {
    setprop("controls/flight/flaps", 0);
    setprop("sim/flaps/current-setting", 0);
    setprop("sim/messages/copilot", "Do you want to destroy the flaps due to overspeed (max 260)????");    
  }
});
...
setlistener("controls/flight/slats", func
 { 
 if ((getprop("controls/flight/slats") > 0  ) and (getprop("/instrumentation/airspeed-indicator/indicated-speed-kt") > 280  ))
  {
    setprop("controls/flight/slats", 0);
    setprop("sim/messages/copilot", "Do you want to destroy the slats due to overspeed (max 280)????");    
  }
});
...
setlistener("controls/flight/spoilers", func
 { 
 if ((getprop("controls/flight/spoilers") > 0  ) and (getprop("/instrumentation/airspeed-indicator/indicated-speed-kt") > 360  ))
  {
    setprop("controls/flight/spoilers", 0);
    setprop("sim/spoilers/current-setting", 0);
    setprop("sim/messages/copilot", "Do you want to destroy the spoilers due to overspeed (max 360)????");    
  }
});
...
#####################################################################################################
# Tutorial help  variables
#
setprop("environment/magnetic-variation-deg-tutorial", getprop("environment/magnetic-variation-deg") + getprop("instrumentation/magnetic-compass/pitch-offset-deg"));

setlistener("instrumentation/magnetic-compass/pitch-offset-deg", func
 { 
setprop("environment/magnetic-variation-deg-tutorial", getprop("environment/magnetic-variation-deg") + getprop("instrumentation/magnetic-compass/pitch-offset-deg"));
});
